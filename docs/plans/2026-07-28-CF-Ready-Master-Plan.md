...
| D-145 | Verificare la cifra di controllo del CF provvisorio numerico a 11 cifre e rifiutare esplicitamente `00000000000`. | La cifra di controllo non attesta che il codice esista o appartenga a qualcuno: rifiuta soltanto sequenze che non possono rispettare il formato. La validazione resta formale e non anagrafica. Su un campione esplorativo di 200.000 casi il controllo ha rilevato il 100% degli errori di una cifra, il 97,7% delle trasposizioni adiacenti considerate e ha rifiutato l’89,9% delle sequenze casuali prima accettate; i test usano casi deterministici, perché le misure dipendono dal campionamento e Luhn non rileva gli scambi `09`/`90`. Deciso dall’owner l’8 settembre 2026 per la `1.4.0`; supera D-017. |
...
| D-162 | La pubblicazione elimina attese ridondanti e interruzioni evitabili. Deploy Development, promozione e merge Production accettano `verify`, `e2e` e `coverage` dell'HEAD PR quando il commit unito su `develop` ne conserva il tree, mentre un esito concluso e non verde sul commit stesso prevale e `ci-policy` resta legato allo SHA candidato. Prima del push il coordinatore esegue in locale gli stessi comandi dei job `verify` e `coverage` della corsia; prima del merge su `develop` attende i check obbligatori e la fine di promozioni e deploy di altre pubblicazioni; prima della promozione collega a `develop` un'eventuale promozione senza deploy tramite `reconcile-develop`. Il job aggregato `mutation` rende bloccante su `develop` e `main` la soglia mutation di D-141 per i diff che toccano domini critici. Il preflight ritenta tre volte le letture provider, un test di policy ferma nella PR le migrazioni che richiedono due fasi, e un job giornaliero verifica lo schema Function. Dependabot raggruppa le major delle librerie app Shopify, e `esbuild` e `workerd` sono approvati per nome in `allowScripts`. | Sulle 18 promozioni dal 13 al 23 settembre 2026 il ciclo mediano durava 13 minuti: 177 secondi del deploy Development attendevano la CI di push che ripeteva gate già verdi sullo stesso tree, due PR sono fallite sulla coverage non eseguita in locale, tre deploy Development si sono fermati su una migrazione in due fasi, una lettura Shopify transitoria e una deriva dello schema, e due promozioni hanno rieseguito la CI perché `develop` era avanzato. Le major Shopify separate e le versioni esatte in `allowScripts` hanno generato 24 run Dependabot falliti in due giorni. La soglia mutation non era un required check e il merge avveniva prima della matrice: il gate aggregato la rende effettiva al costo di attendere la mutation nelle sole PR sui domini critici. Per `esbuild` e `workerd` il pin non protegge nulla di concreto, perché i loro binari vengono comunque eseguiti in build e test. Deciso il 23 settembre 2026. |
| D-163 | I gate mutation di PR e push usano il report incrementale Stryker di ciascun dominio, salvato in cache soltanto dai push su `develop` e dalla campagna settimanale, che forza il run completo. Le PR leggono la base più recente e non la sovrascrivono. Soglia, domini e campagna restano quelli di D-141. | Con D-162 la mutation blocca il merge, e la matrice `billing` durava fino a 11 minuti. Misurato in locale sul dominio webhook: il run completo impiega 53 secondi, quello incrementale senza modifiche 4, con 437 risultati su 437 riusati e lo stesso punteggio. Stryker non rileva i cambi in moduli importati ma non mutati: la campagna completa settimanale limita a una settimana l'esposizione a un risultato riusato non più valido. Deciso il 23 settembre 2026. |
| D-164 | Il ciclo periodico ogni cinque minuti rinnova con il refresh token, prima della riconciliazione billing, fino a tre token offline di store attivi che scadono entro venti minuti, partendo dalla scadenza più recente; un token scaduto da oltre un giorno non viene più ritentato. Un rinnovo fallito registra `offline_token_refresh_failed` e non ferma le fasi successive; l'apertura seguente torna al token exchange. | Con `expiringOfflineAccessTokens` il token offline dura un'ora e la libreria lo considera scaduto negli ultimi cinque minuti: quasi ogni apertura reale del merchant eseguiva token exchange, salvataggio della sessione e `afterAuth` prima dell'HTML, circa 500 ms nel percorso LCP che Shopify misura per Built for Shopify. Il rinnovo anticipato usa il grant ufficiale di `@shopify/shopify-api` e lo storage cifrato esistente, senza nuovi scope, provider o dati. Tre store per esecuzione restano nel limite CPU di §18.4; l'ordine per scadenza decrescente fa scendere in coda una sessione che continua a fallire. Deciso dall'owner il 24 settembre 2026. |
| D-165 | Il reporter Web Vitals è uno script inline del documento `/app`, registrato durante il parsing e non dopo l'idratazione. Invia con `sendBeacon` (ripiego `fetch` con `keepalive`) a `/performance`, rotta fuori dal layout autenticato che accetta soltanto report con una firma HMAC dello store valida 24 ore, emessa dal loader autenticato con una chiave derivata via HKDF da `TRIAL_LEDGER_HMAC_KEY`. Dentro `s-app-window` non si registra. | Shopify conta per Built for Shopify caricamenti che non arrivavano a D1: App Bridge consegna il report solo ai callback già registrati, e la POST richiedeva un session token asincrono, valido un minuto, spesso non più ottenibile mentre l'iframe si chiude. Il 22 settembre 2026 il Dev Dashboard mostrava un caricamento da 4.528 ms assente in D1. La firma prova lo store senza session token; route e `Server-Timing` restano quelli del documento iniziale (D-142) e l'allowlist resta al server. Nessun ripiego al `DOMContentLoaded`: in una App Window App Bridge adotta lo `shopify` della finestra principale e il callback della Home verrebbe sostituito. Deciso dall'owner il 24 settembre 2026. |
| D-166 | L'App URL è `/app` in Development e Production; `redirect_urls` e `SHOPIFY_APP_URL` restano sull'origine. La radice `/` continua a inoltrare a `/app` per link e configurazioni precedenti. La voce Home resta visibile nella barra laterale. | Con l'App URL sulla radice, ogni apertura dall'Admin passava da un 302 prima dell'HTML: circa 50-180 ms dentro l'LCP misurato da Shopify, perché la navigazione dell'iframe parte dalla richiesta iniziale. Il redirect era il residuo del form di login del template rimosso con D-128, non una scelta di prestazioni. Aggiorna la parte di D-130 per cui il titolo dell'app usava la radice. Da verificare su Development prima della promozione: apertura dal menu, link diretti alle pagine, ricarica di una pagina interna, voce Home e titolo nel menu, onboarding in `s-app-window`. Se la voce Home sparisce o l'Admin non conserva i link diretti, si torna all'App URL sulla radice. Deciso dall'owner il 24 settembre 2026. |
| D-167 | La Home disegna subito l'ultimo stato noto in D1 (Validation, piano, prova, omaggio, ultima configurazione salvata) e riceve in streaming, nella stessa risposta, lo stato riconciliato con Shopify, che lo sostituisce senza rimontare la pagina. Finché Shopify non conferma, Validation e piano mostrano “Verifica in corso…” / “Checking with Shopify…” e le azioni restano disabilitate; se la riconciliazione fallisce o supera il timeout di streaming compare un avviso con “Riprova”, il checkout resta fail-open. Completamento automatico dell'onboarding e richiesta di recensione partono solo dallo stato confermato; la riconciliazione prosegue con `waitUntil` anche se il merchant chiude la pagina. Per gli script Shopify del `<head>` resta l'header `Link` preload che la libreria Shopify già invia; il download anticipato prima del Worker richiede Early Hints attivo sulla zona `cfready.it`, impostazione che attiva l'owner. | Sulla Home quasi tutto il tempo server era `reconcile_total` (circa 270-320 ms su Production), eseguito prima dell'HTML: LCP ≈ FCP, quindi l'attesa entrava per intero nell'LCP misurato da Shopify per Built for Shopify. Il primo paint contiene già l'elemento LCP e la conferma cambia solo testi e stati, così LCP e CLS restano sul documento iniziale. `reconcile_total` e le durate Shopify escono dal `Server-Timing` della Home, inviato prima della conferma. Early Hints non è verificabile su Development (`workers.dev`, fuori zona). Deciso dall'owner il 24 settembre 2026. |
...
| Concessione omaggio permanente | D1, assegnazione manuale owner | Copiata nel metafield come diritto senza scadenza |
| Onboarding e stato UI | D1 | Nessuna |
| Sessioni e token | D1 cifrato | Nessuna |

La UI non deve presentare come certo uno stato locale vecchio: all’apertura della Home e prima di ogni mutazione rilevante va eseguita una riconciliazione con Shopify. La Home può mostrare subito l’ultimo stato noto solo come “in verifica”, con le azioni disabilitate fino alla conferma (D-167).
...
readback locale rendono innocui replay, paginazione e doppia osservazione dello
stesso evento. `billing_events` conserva anche piano e stato precedenti per
distinguere un nuovo acquisto da un passaggio tra piani. Al primo avvio del cursore
billing vengono recuperate soltanto le transizioni successive all’attivazione
dell’outbox, evitando notifiche retroattive per acquisti storici.
...
| `address2_form_mode` | `required`, `optional`, `hidden` o `NULL` finché il merchant non sceglie |
...
senza leggere D1 quando l'isolate ha appena confermato lo stesso Paese (D-169).
...
Home resta una voce visibile del menu e `/app` vi compare una volta sola, senza
`rel="home"`. Il titolo dell’app usa la rotta predefinita `/`, che dopo la
rimozione della pagina di accesso inoltra a `/app` conservando i parametri della
richiesta. Dichiarare invece `/app` due volte, una visibile e una nascosta con
`rel="home"`, rompeva il menu dell’Admin (D-128, D-130).
...
variante obbligatoria, facoltativa o non mostrata. CF Ready non può leggere automaticamente
...
**Qualificazione del 20 settembre 2026.** La baseline Production non era
compatibile con il piano Free: su circa 15.990 invocazioni nelle 24 ore, CPU p90
22,63 ms e p99 36,67 ms. Tail minimizzati hanno attribuito la coda al percorso webhook, in
particolare `SHOP_UPDATE`, che inizializzava React Router e Shopify Admin anche
quando terminava dopo HMAC, parsing e una lettura D1. La `1.12.0` sposta
le cinque route webhook nell'ingresso Worker leggero e carica router, consumer,
cron e controllo owner soltanto sul rispettivo evento. Il readback immediato
Production ha osservato i webhook a CPU p90 3 ms e p99 4 ms; il campione breve
non sostituisce il percentile su 24 ore. Il candidato successivo porta il ciclo
owner da ogni minuto a ogni cinque minuti, riducendo dell'80% le invocazioni
periodiche senza cambiare la CPU per esecuzione. Ricevuta e limite residuo:
...
provenienza dei commit e riusa i gate già verdi di `develop`. Il job `mutation`
aggrega la matrice dei domini critici in un check stabile. Governance,
...
risolte, protezioni applicate agli admin e `verify`, `coverage`, `mutation`,
`react-doctor`, `dependency-review`, `e2e` e `ci-policy` come required checks; su `main` è
...
mutant. I gate mutation delle PR e dei push riusano il report incrementale
dell'ultimo run su `develop` e ritestano solo i mutanti di file o test cambiati;
la campagna completa forza ogni mutante e rinnova quella base (D-163). La campagna
...
- percorso billing fino alla conferma Shopify, senza approvare una charge Production;
- risultati attesi;
- limitazioni wallet;
- contatto rapido.
