...
| D-116 | Restare sull’ultima React Router 7 compatibile con Shopify, applicare il backport `7.18.2` e non abilitare le API RSC instabili finché Shopify non supporta React Router 8. | `GHSA-qwww-vcr4-c8h2` riguarda soltanto i percorsi RSC instabili, non usati da CF Ready. Il database advisory può continuare a segnalarla finché non recepisce il backport: l’eccezione resta puntuale e va rimossa appena l’intervallo globale viene corretto. |
