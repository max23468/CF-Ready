...
| **Riconferma schema sul candidato `1.0.0`** | fonti ufficiali rilette il 26 agosto; `npm run verify:function-schema` è riuscito in locale ed è ora un gate autenticato esplicito dei workflow Development e Production; la corsia Development di sola verifica schema/readback non ripubblica la versione immutabile | ⚠️ run schema/readback Development richiesto prima della promozione |
