...
SELECT
  ...
  COALESCE(SUM(CASE WHEN app_state.onboarding_status = 'completed' THEN 1 ELSE 0 END), 0) AS onboarding_completed,
  COALESCE(SUM(CASE WHEN app_state.validation_enabled = 1 THEN 1 ELSE 0 END), 0) AS validations_enabled,
  COALESCE(SUM(CASE WHEN app_state.last_error_code IS NOT NULL THEN 1 ELSE 0 END), 0) AS stores_with_open_error,
  COALESCE(SUM(CASE
  ...
  COALESCE(SUM(CASE WHEN complimentary_entitlements.status = 'active' THEN 1 ELSE 0 END), 0) AS complimentary_stores,
