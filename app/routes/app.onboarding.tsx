      ...
      <s-section>
        <s-stack direction="block" gap="base">
          ...
          <s-stack direction="inline" gap="base">
            {step > 1 ? (
              <s-button disabled={busy} onClick={() => go("back")}>
                {t.onboarding.back}
              </s-button>
            ) : null}
            {step === 4 ? (
              <>
                <s-button variant="primary" disabled={busy} onClick={() => activate(go)}>
                  {t.onboarding.activate}
                </s-button>
                <s-button disabled={busy} onClick={() => finish(go)}>
              ...
              <s-button
                ...
                disabled={busy}
                onClick={() =>
                  step === 2 ? go("rules", { taxCode: rules.taxCode, pec: rules.pec }) : go("next")
                }
              >
                {t.onboarding.next}
              </s-button>
...
// La dichiarazione FR-058 viaggia solo quando il blocco è stato reso, come nelle Regole.
function declarationFields() {
  const checkbox = document.querySelector<HTMLInputElement & { checked: boolean }>(
    '[name="address2"]',
  );
  if (!checkbox) return {};
  return { address2Shown: "1", ...(checkbox.checked ? { address2: "declared" } : {}) };
}

function activate(go: (intent: string, extra?: Record<string, string>) => void) {
  go("activate", declarationFields());
}

function finish(go: (intent: string, extra?: Record<string, string>) => void) {
  go("finish", declarationFields());
}
